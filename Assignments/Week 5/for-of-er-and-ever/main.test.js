...
const cubedNumbers = cubeAll(numbers);
console.log(cubedNumbers); // Output: [1, 8, 27, 64, 125]


function addNoises(animals) {
  const result = []; // Define the result array
  //for (let animal of animals) {
      if (animal === 'cat') {
          result.push('cat says meow');
      } else if (animal === 'dog') {
          result.push('dog says woof');
      } else if (animal === 'cow') {
          result.push('cow says moo');
      } else {
          result.push('unknown animal');
      }
  }
...
// Test cases
console.log(addNoises(['cat', 'dog', 'cow'])); // Output: ['cat says meow', 'dog says woof', 'cow says moo']
console.log(addNoises(['dog', 'cow', 'bird'])); // Output: ['dog says woof', 'cow says moo', 'unknown animal']


function womenOnly(namesAndGenders) {
  return namesAndGenders.filter(name => name.endsWith('F'));
}

// Test cases
const namesAndGenders = ['John M', 'Jane F', 'Alice F', 'Bob M'];
console.log(womenOnly(namesAndGenders)); // Output: ['Jane F', 'Alice F']
